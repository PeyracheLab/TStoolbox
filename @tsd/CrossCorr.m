...
%
% v2.0 Luke Sjulson, Aug 2017. Function runs, but output not validated.
...
warning('v2.0 of this function has not been tested thoroughly'); % remove this after tested fully
...
if isa(tsb, 'tsd')
    ...
    switch errors
        case 'none'
            [C, B] = CrossCorr_c(Range(tsa), Range(T2), binsize, ...
                nbins, fix_boundaries);
            Cr = tsd(B, bs * C);
        case 'std'
            [C, B, C2] = CrossCorr_c(Range(tsa), Range(T2), binsize, ...
                nbins, fix_boundaries);
            Cr = tsd(B, bs * [C C2]);
        case 'sem'
            [C, B, C2] = CrossCorr_c(Range(tsa), Range(T2), binsize, ...
